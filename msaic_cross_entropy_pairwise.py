...
out_ph = tf.placeholder(dtype=tf.float32,shape=[None,10],name="Answers")
...
o11 = tf.layers.dense(o1_dp,128,name="head1_o11",activation="relu")
o11_dp = tf.layers.dropout(inputs=o11,rate=drop_prob)
o111 = tf.layers.dense(o11_dp,128,activation="relu")
o111_dp = tf.layers.dropout(o111,rate=drop_prob)
out1 = tf.layers.dense(o111_dp,1,name="head1_out",activation=None)  # [Batch,10,1]
...
o22 = tf.layers.dense(o2_dp,128,name="head2_o11",activation="relu")
o22_dp = tf.layers.dropout(inputs=o22,rate=drop_prob)
o222 = tf.layers.dense(o22_dp,128,activation="relu")
o222_dp = tf.layers.dropout(o222,rate=drop_prob)
out2 = tf.layers.dense(o222_dp,1,name="head2_out",activation=None)  # [Batch,10,1]

# Output
#  y_out = tf.nn.softmax(out1+out2,axis=1)
#  y_out = tf.sigmoid(out1 + out2)
y_out = out1 + out2 # [Batch,10,1]
y_shape = tf.shape(y_out)
yout = tf.reshape(y_out, shape=(y_shape[0], y_shape[1]))
#  Define Optimizer
optimizer = tf.train.AdamOptimizer(learning_rate=0.0001)
loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=out_ph, logits=yout))
...
with tf.Session() as sess:
    ...
    for i in range(Epochs+1):
        for j in range(0,100): # Paragraphs from 100 to 104 are used for validation
            ...
            training_loss, _ = sess.run([loss, train_op], feed_dict={ques_ph: Ques1,
                                                                          para_ph: Paras,
                                                                          drop_prob: 0.2,
                                                                          out_ph: Ans})
